...
COMPARED = (
    "citation_coverage", "faithfulness", "abstention_recall", "numerical_agreement",
    "driver_recall", "prompt_injection_block_rate",
)


def _deltas(champion: ScoreCard, candidate: ScoreCard) -> dict[str, float]:
    return {
        name: round(getattr(candidate, name) - getattr(champion, name), 6)
        for name in COMPARED
    }


def compare_adapters(champion_scores: dict[str, Any], candidate_scores: dict[str, Any],
                     gates: ReleaseGates) -> dict[str, Any]:
