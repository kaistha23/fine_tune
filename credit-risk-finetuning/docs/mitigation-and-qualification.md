...
A numeric threshold claim with a verified `derivation` does not need a separate numeric
review waiver. The checker resolves the metric from `calculated_metrics`, requires valid
status and exact units, matches the observed value, resolves the threshold citation, and
