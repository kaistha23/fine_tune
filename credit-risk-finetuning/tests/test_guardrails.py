...
class FactsheetCitationTests(unittest.TestCase):
    ...
    def _evidence(self) -> list[Evidence]:
        return [Evidence(
            evidence_id="SAMA-DOC-4#7.2", jurisdiction=Jurisdiction.SAMA,
            document_id="SAMA-DOC-4", document_version="1.0", section="7.2",
    ...
    def _response(self, evidence_ids: list[str]) -> CreditResponse:
        return CreditResponse(
            ...
            executive_summary="Position summary.",
            facts=[SupportedClaim(statement="The obligor is in stage 1 with 0 days past due.",
                                  evidence_ids=evidence_ids)],
            human_approval_required=True)

    def test_the_factsheet_case_id_is_a_valid_citation(self) -> None:
        result = validate_output(self._response(["CASE-OBL-0008-2026-01-15"]),
                                 self._evidence(),
    ...
    def test_a_different_case_id_is_still_refused(self) -> None:
        ...
        result = validate_output(self._response(["CASE-OBL-9999-2026-01-15"]),
                                 self._evidence(),
                                 factsheet_case_id="CASE-OBL-0008-2026-01-15")
        self.assertFalse(result.passed)
    ...
    def test_policy_citations_still_work_alongside_it(self) -> None:
        result = validate_output(self._response(["SAMA-DOC-4#7.2"]), self._evidence(),
                                 factsheet_case_id="CASE-OBL-0008-2026-01-15")
        self.assertTrue(result.passed, result.failures)

    def test_an_uncited_fact_is_still_refused(self) -> None:
        result = validate_output(self._response([]), self._evidence(),
                                 factsheet_case_id="CASE-OBL-0008-2026-01-15")
        self.assertFalse(result.passed)
