...
def plan(**overrides) -> QueryPlan:
    base = dict(
        ...
        obligor_id="OBL-0008", date_from=date(2025, 1, 31), date_to=date(2025, 3, 31),
...
def rows() -> list[dict]:
    return [
        {"observation_date": "2025-01-31", "obligor_id": "OBL-0008",
         "current_assets": 120.0, "current_liabilities": 100.0,
         "outstanding": 500.0, "facility_limit": 1000.0,
         "pit_pd": 0.02, "lgd": 0.4, "ead": 520.0, "ecl": 4.16,
         "stage": 1, "days_past_due": 0, "internal_rating": "3"},
        {"observation_date": "2025-02-28", "obligor_id": "OBL-0008",
         "current_assets": 110.0, "current_liabilities": 100.0,
         "outstanding": 650.0, "facility_limit": 1000.0,
         "pit_pd": 0.035, "lgd": 0.4, "ead": 670.0, "ecl": 9.38,
         "stage": 2, "days_past_due": 35, "internal_rating": "5"},
        {"observation_date": "2025-03-31", "obligor_id": "OBL-0008",
         "current_assets": 90.0, "current_liabilities": 100.0,
         "outstanding": 800.0, "facility_limit": 1000.0,
         "pit_pd": 0.05, "lgd": 0.4, "ead": 820.0, "ecl": 16.4,
         "stage": 2, "days_past_due": 41, "internal_rating": "6"},
    ]
...
class FactsheetTests(unittest.TestCase):
    ...
    def test_model_outputs_are_reported_never_recomputed(self) -> None:
        ...
        self.assertNotIn("ecl", {k for k, v in sheet.calculated_metrics.items()
                                 if v.formula_id})
    ...
    def test_missing_inputs_are_surfaced_not_silently_dropped(self) -> None:
        ...
        self.assertEqual(sheet.calculated_metrics["current_ratio"].validation_status,
                         "warning")
        self.assertIn("current_assets", sheet.missing_information)

    def test_stale_data_is_flagged(self) -> None:
        sheet = build_factsheet(rows(), plan(as_of_date=date(2026, 1, 15),
...
class DatasetFormatTests(unittest.TestCase):
    def test_training_line_is_an_object_keyed_by_messages(self) -> None:
        record = build_sft_record(
            {"obligor_id": "OBL-0008", "portfolio": "corporate", "jurisdiction": "SAMA"},
            "target text", "credit_deterioration")
        ...
        self.assertEqual([m["role"] for m in parsed["messages"]],
    ...
    def test_provenance_survives_the_round_trip(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            source.write_text(json.dumps({
                "case": {"obligor_id": "OBL-0008", "portfolio": "corporate",
                         "jurisdiction": "SAMA"},
                "target": "target text",
            ...
            out = Path(tmp) / "out"
            subprocess.run(
                [sys.executable, "-m", "credit_risk.dataset", str(source), str(out)],
                cwd=ROOT, check=True, capture_output=True,
            )
            written = [p for p in out.glob("*.jsonl")
                       if not p.name.endswith(".provenance.jsonl")]
            ...
            prov = [line for p in out.glob("*.provenance.jsonl")
                    for line in p.read_text(encoding="utf-8").splitlines()]
            self.assertEqual(len(prov), 1)
            meta = json.loads(prov[0])
            for key in ("example_id", "portfolio", "task_type", "jurisdiction",
